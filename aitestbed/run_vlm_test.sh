...
SCENARIO="realtime_video_understanding" # realtime_video_understanding, chat_token
RUNS=2
PROFILE_INTERVAL=20

mkdir -p results/reports
mkdir -p results/captures/vlm
mkdir -p logs

# ============== clean old results ==============
cleanup_profile_files() {
    local profile=$1
    local capture_dir="results/captures/vlm/${profile}"
    local report_file="results/reports/experiment_report_${SCENARIO}_${profile}.json"
    local db_file="logs/traffic_logs_vlm_${profile}.db"
    ...
    for i in $(seq 1 25); do
        local json_file="examples/assets/dataset/sample_${i}/questions_results_${profile}.json"
        if [ -f "$json_file" ]; then
            rm -f "$json_file"
...
# Source: results/captures/vlm (contains all profile subdirs generated in the loop)
