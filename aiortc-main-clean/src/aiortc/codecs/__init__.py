...
from .tokenId import TokenIdDecoder, TokenIdEncoder, AsyncTokenIdDecoder, tokenId_depayload
...
def init_codecs() -> None:
    ...
    # Add token index codec that mimics VP8 format but carries token data
    add_video_codec("video/tokenId")
    add_video_codec("video/tokenIdTransparent")
    add_video_codec("video/tokenIdLLM")
...
def depayload(codec: RTCRtpCodecParameters, payload: bytes) -> bytes:
    ...
    elif codec.name == "tokenId" or codec.name == "tokenIdTransparent" or codec.name == "tokenIdLLM":
        return tokenId_depayload(payload)
    else:
        return payload
...
def get_decoder(codec: RTCRtpCodecParameters) -> Decoder:
    ...
    if mimeType == "audio/g722":
        ...
        return Vp8Decoder()
    elif mimeType == "video/tokenid":
        return TokenIdDecoder(decMode="detokenize")
    elif mimeType ==  "video/tokenidtransparent":
        return TokenIdDecoder(decMode="transparent")
    elif mimeType == "video/tokenidllm":
        return AsyncTokenIdDecoder()
    else:
...
def get_encoder(codec: RTCRtpCodecParameters) -> Encoder:
    ...
    if mimeType == "audio/g722":
        ...
        return Vp8Encoder()
    elif mimeType == "video/tokenid" or mimeType == "video/tokenidtransparent" or mimeType == "video/tokenidllm":
        return TokenIdEncoder()
    else:
