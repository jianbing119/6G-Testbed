...
class JitterBuffer:
    ...
    def add(self, packet: RtpPacket, codecName) -> tuple[bool, Optional[JitterFrame]]:
        ...
        return pli_flag, self._remove_frame(packet.sequence_number, codecName)

    def _remove_frame(self, sequence_number: int, codecName) -> Optional[JitterFrame]:
        frame = None
        frames = 0
        ...
        for count in range(self.capacity):
            ...
            if "token" in codecName:
                if packet is None:
                    continue # for lossy transmission
                if (timestamp is None) and (packet is not None): # for lossy tx
                    timestamp = packet.timestamp
                elif (packet.timestamp != timestamp) and (timestamp is not None): # for lossy tx
                    if frame is None:
                        frame = JitterFrame(
